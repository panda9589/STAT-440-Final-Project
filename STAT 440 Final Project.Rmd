---
title: "Optimal Parameter Estimation and Prediction in a Poisson Time Series Model Using Monte Carlo Maximum Likelihood and Newton's Method"
author: Yingbo Tong
output:
...
In this project, a Poisson time series model with latent variales was
fit to count data, a common task in the epidemiology of non-infectious
diseases. The model consists of a log-linear relation with latent
variables controlling the trend over time. Given the data set
$y = (y_1, ..., y_n)^T$, we aim to estimate parameters $\beta_0$ and
$\sigma^2$, and predict the latent variables $u = (u_1, ..., u_n)^T$.
Using all the predicted values, a point estimation of the poisson
dataset can be generated, along with a confidence interval for said
estimate.
...
entire vector $u$ set to $\hat{\beta}_0$. The iterative updates provided
estimates for $u$, and is then illustrated with the plot
below:![](images/clipboard-749681816.png)
...
making the running time of the code lengthy. We compiled the following
images for visualizing the impacts of different sizes of $L$:

![](images/clipboard-3604045097.png)

![](images/clipboard-2541730748.png)

![](images/clipboard-2979191514.png)
